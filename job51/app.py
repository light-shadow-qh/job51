...
def query():
    conn = pymysql.connect(host="localhost", user=MYSQL_USER, password=MYSQL_PWD, database=MYSQL_DB, charset="utf8")
...
def get_data():
    ...
    for keyword in keywords:
        ...
        df_type = df[df['keyword'] == keyword].groupby('company_type').size()
        company_types = df_type.index.tolist()
        company_values = df_type.values.tolist()
        for t, v in zip(company_types, company_values):
            if t == '':
                continue
        ...
        df_size = df[df['keyword'] == keyword].groupby('company_size').size()
        company_size_names = df_size.index.tolist()
        company_size_values = df_size.values.tolist()
        for s, v in zip(company_size_names, company_size_values):
            if s == '':
                continue
            data['company_size'].append([
                s, v
            ])
        df['province'] = df[df['keyword'] == keyword]['workplace'].apply(lambda x: x.split('-')[0])
        df_province = df[df['keyword'] == keyword].groupby('province').size()
        company_province_names = df_province.index.tolist()
        company_province_values = df_province.values.tolist()
        for n, v in zip(company_province_names, company_province_values):
            if n == '':
                continue
        ...
        job_info_list = df[(df['keyword'] == keyword)]['job_type'].head(2000).values.tolist()
        words_list = []
        for job_info in job_info_list:
            print(job_info)
            with open('stopwords.txt', 'r', encoding='utf-8') as f:
                stops_words = [line.strip() for line in f.readlines()]
            words = [i.word for i in psg.cut(str(job_info)) if i.flag.startswith('n')]
            words = list(filter(lambda x: x not in stops_words, words))
    ...
    return json.dumps(data_list, ensure_ascii=False) #如果有中文的话，就需要ensure_ascii=False
